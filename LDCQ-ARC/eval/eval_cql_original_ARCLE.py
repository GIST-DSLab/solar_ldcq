"""
Original CQL (Conservative Q-Learning) Evaluation Script for ARCLE
...
def evaluate_task(env, cql_agent, loader, task_idx, max_grid_size=10, max_steps=50, device='cuda'):
    """Evaluate one task with CQL"""
    ...
    while not done and step_count < max_steps:
        ...
        # Get action from CQL (operation, x, y, h, w)
        operation, x, y, h, w = cql_agent.get_action(state, clip, in_grid, pair_in, pair_out)
...
def main(args):
    ...
    if isinstance(checkpoint, OriginalCQL):
        # Full model saved
        cql_agent = checkpoint
        cql_agent.to(device)
    else:
        # State dicts saved
        cql_agent = OriginalCQL(
            ...
            tau=args.tau,
            lr=args.lr,
            cql_alpha=checkpoint.get('cql_alpha', args.cql_alpha),
        ...
        cql_agent.q_net_0.load_state_dict(checkpoint['q_net_0_state_dict'], strict=False)
        cql_agent.q_net_1.load_state_dict(checkpoint['q_net_1_state_dict'], strict=False)

        if 'target_net_0_state_dict' in checkpoint:
            cql_agent.target_net_0.load_state_dict(checkpoint['target_net_0_state_dict'], strict=False)
            cql_agent.target_net_1.load_state_dict(checkpoint['target_net_1_state_dict'], strict=False)

        print(f"Loaded checkpoint from {args.checkpoint_path}")
        print(f"Checkpoint epoch: {checkpoint['epoch']}, loss: {checkpoint['loss']:.6f}, CQL alpha: {checkpoint.get('cql_alpha', 'N/A')}")

    cql_agent.q_net_0.eval()
    cql_agent.q_net_1.eval()
    ...
    print("="*80)
    print("OVERALL RESULTS (Original CQL - Joint Action Space)")
    print("="*80)
    ...
    results_path = os.path.join(args.output_dir, 'cql_results.json')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint_path', type=str, required=True, help='Path to CQL checkpoint')
    ...
    # CQL params
    parser.add_argument('--cql_alpha', type=float, default=1.0, help='CQL alpha (for model init)')
    parser.add_argument('--cql_temperature', type=float, default=1.0, help='CQL temperature')
