"""
Original IQL (Implicit Q-Learning) Evaluation Script for ARCLE
...
from models.iql_original import OriginalIQL
...
def evaluate_task(env, iql_agent, loader, task_idx, max_grid_size=10, max_steps=50, device='cuda', use_policy=True):
    """Evaluate one task with IQL

    Args:
        use_policy: If True, use policy network for action selection.
                   If False, use Q-network (argmax Q) for action selection.
    """
    ...
    while not done and step_count < max_steps:
        ...
        # Get clipboard state from environment
        clip_h, clip_w = obs['clip_dim']
        current_clip = np.full((max_grid_size, max_grid_size), 10, dtype=np.float32)
        current_clip[:clip_h, :clip_w] = obs['clip'][:clip_h, :clip_w]
        clip = torch.FloatTensor(current_clip).unsqueeze(0).unsqueeze(0).to(device)  # (1, 1, H, W)

        # Get action from IQL
        if use_policy:
            # Use policy network (recommended for IQL)
            operation, x, y, h, w = iql_agent.get_action(state, clip, in_grid, pair_in, pair_out, deterministic=True)
        else:
            # Use Q-network (argmax Q)
            operation, x, y, h, w = iql_agent.get_action_from_q(state, clip, in_grid, pair_in, pair_out)

        print(f"Step: {step_count}| op: {operation}, x: {x}, y: {y}, h : {h}, w : {w}")
    ...
    # Check if reached the answer (final grid matches target)
    final_h, final_w = obs['grid_dim']
    final_grid = obs['grid'][:final_h, :final_w]
...
def main(args):
    ...
    # Check if checkpoint is a full model or just state dicts
    checkpoint = torch.load(args.checkpoint_path, map_location=device)

    if isinstance(checkpoint, OriginalIQL):
        # Full model saved
        iql_agent = checkpoint
        iql_agent.device = device
        iql_agent.q_net_0 = iql_agent.q_net_0.to(device)
        iql_agent.q_net_1 = iql_agent.q_net_1.to(device)
        iql_agent.v_net = iql_agent.v_net.to(device)
        iql_agent.policy_net = iql_agent.policy_net.to(device)
        if iql_agent.target_q_net_0 is not None:
            iql_agent.target_q_net_0 = iql_agent.target_q_net_0.to(device)
            iql_agent.target_q_net_1 = iql_agent.target_q_net_1.to(device)
    else:
        # State dicts saved
        iql_agent = OriginalIQL(
            a_dim=args.a_dim,
            h_dim=args.h_dim,
            ...
            tau=args.tau,
            lr=args.lr,
            iql_tau=checkpoint.get('iql_tau', args.iql_tau),
            beta=checkpoint.get('beta', args.beta),
            use_positional_encoding=bool(args.use_positional_encoding),
            device=device,
            scheduler_type='step'
        )

        iql_agent.q_net_0.load_state_dict(checkpoint['q_net_0_state_dict'], strict=False)
        iql_agent.q_net_1.load_state_dict(checkpoint['q_net_1_state_dict'], strict=False)
        iql_agent.v_net.load_state_dict(checkpoint['v_net_state_dict'], strict=False)
        iql_agent.policy_net.load_state_dict(checkpoint['policy_net_state_dict'], strict=False)

        if 'target_q_net_0_state_dict' in checkpoint:
            iql_agent.target_q_net_0.load_state_dict(checkpoint['target_q_net_0_state_dict'], strict=False)
            iql_agent.target_q_net_1.load_state_dict(checkpoint['target_q_net_1_state_dict'], strict=False)

        print(f"Loaded checkpoint from {args.checkpoint_path}")
        print(f"Checkpoint epoch: {checkpoint['epoch']}, loss: {checkpoint['loss']:.6f}")
        print(f"IQL tau: {checkpoint.get('iql_tau', 'N/A')}, Beta: {checkpoint.get('beta', 'N/A')}")

    iql_agent.q_net_0.eval()
    iql_agent.q_net_1.eval()
    iql_agent.v_net.eval()
    ...
    loader = ARC_Dataloader(args.data_path, train=False)

    # Create environment
    print("Creating ARCLE environment...")
    env = gym.make('ARCLE/O2ARCv2Env-v0', data_loader=loader, max_grid_size=(args.max_grid_size, args.max_grid_size), max_trial=3)
    ...
    use_policy = bool(args.use_policy)
    action_source = "Policy" if use_policy else "Q-network"
    print(f"\nEvaluating {total_tasks} tasks using {action_source}...")

    for task_idx in range(total_tasks):
        try:
            trajectory_data, is_successful, reach_answer = evaluate_task(
                env, iql_agent, loader, task_idx,
                max_grid_size=args.max_grid_size,
            ...
            total_runs = task_idx + 1
            print(f"env_step : {len(trajectory_data['steps'])}")
            print(f"Total score: {success_count} out of {total_runs} i.e. Acc: {(success_count / total_runs) * 100}%")
            print(f"Reach answer: {reach_count} out of {total_runs} i.e. Acc: {(reach_count / total_runs) * 100}%")
    ...
    print("="*80)
    print(f"OVERALL RESULTS (Original IQL - Joint Action Space, {action_source})")
    print("="*80)
    print(f"Total tasks: {total_tasks}")
    print(f"Submit: {success_count}/{total_tasks} ({success_rate:.2%})")
    print(f"Reach:  {reach_count}/{total_tasks} ({reach_rate:.2%})")
    print(f"Average steps: {avg_steps:.2f}")
    print(f"IQL tau: {args.iql_tau}, Beta: {args.beta}")
    print("="*80)
    ...
    results_path = os.path.join(args.output_dir, 'iql_results.json')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint_path', type=str, required=True, help='Path to IQL checkpoint')
    ...
    # IQL params
    parser.add_argument('--iql_tau', type=float, default=0.7, help='IQL expectile')
    ...
    parser.add_argument('--use_policy', type=int, default=1, help='Use policy network (1) or Q-network (0) for action selection')
