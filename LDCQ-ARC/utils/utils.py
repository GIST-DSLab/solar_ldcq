...
# import ipdb  # Commented out - not available in environment
import random
import pickle
import os
import sys
...
def stable_weighted_log_sum_exp(x,w,sum_dim):
    a = torch.min(x)
    # ipdb.set_trace()  # Commented out - debug line
...
class ARC_Segment_Dataset(Dataset):
    ...
    def __init__(self, data_path, return_concept=False):
        self.data_path = Path(data_path)
        self.return_concept = return_concept

        file_list = []
        for path, _, files in os.walk(self.data_path, followlinks=True):
            for name in files:
                file_list.append(os.path.join(path, name))

        self.num_dataset = len(file_list)
    ...
    def __getitem__(self, idx):
        ...
        if self.return_concept:
            concept = trace.get('desc', {}).get('concept', '')
            return state, s_T, clip, clip_T, selection, operation, reward, terminated, selection_mask, in_grid, out_grid, ex_in, ex_out, concept
        else:
