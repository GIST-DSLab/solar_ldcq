"""
Original IQL (Implicit Q-Learning) Training Script for ARC tasks
Joint action space (non-factorized): 36 × max_grid_size^4
Supports PER (Prioritized Experience Replay)
Uses same data loading as train_cql_original.py
"""
import os
import sys
import json
...
class IQLPrioritizedBuffer:
    """Prioritized Experience Replay Buffer for IQL"""
...
class ARCTrajectoryDataset:
    """
    Dataset for loading ARC trajectories for IQL training
    Same as train_cql_original.py
    """
...
def train(args):
    ...
    # Create IQL agent
    print(f"\nInitializing Original IQL Agent:")
    ...
    print(f"  IQL tau (expectile): {args.iql_tau}")
    print(f"  Beta (AWR temperature): {args.beta}")
    ...
    if use_per:
        ...
        per_buffer = IQLPrioritizedBuffer(
    ...
    # Start training
    print(f"\n{'='*60}")
    print(f"Starting Original IQL Training (Joint Action Space)")
...
if __name__ == "__main__":
    ...
    # IQL specific
    parser.add_argument('--iql_tau', type=float, default=0.7, help='IQL expectile (0.5=mean, 1.0=max)')
    parser.add_argument('--beta', type=float, default=3.0, help='AWR temperature for policy extraction')
