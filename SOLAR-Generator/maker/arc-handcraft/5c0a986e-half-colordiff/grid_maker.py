    ...
    def parse(self, **kwargs) -> List[Tuple[List[NDArray], List[NDArray], List[NDArray], List[NDArray], Dict]]:
        ...
        while num < num_samples:
            ...
            for i in range(1):
                ...
                remaining_ops = operations[branch_idx:-1]
                remaining_sels = selections[branch_idx:-1]
                for i, (sel, op) in enumerate(zip(remaining_sels, remaining_ops)):
                    if op in p_colors:
                        wrong_color = p_colors[1] if op == p_colors[0] else p_colors[0]
                        rand_operations.append(wrong_color)
                    else:
                        rand_operations.append(op)
                    rand_selections.append(sel)
