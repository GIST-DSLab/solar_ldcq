    ...
    def parse(self, **kwargs) -> List[Tuple[List[NDArray], List[NDArray], List[NDArray], List[NDArray], Dict]]:
        ...
        while num < num_samples:
            ...
            while (j < num_examples+1):
                ...
                step1_diagonal_grid = rand_grid.copy()
                s1 = (x1, y1)
                for i in range(f_len):
                    s1 = (s1[0]-1, s1[1]-1)
                    x, y = s1
                    step1_diagonal_grid[x][y] = rect1_color
                s2 = (x2+1, y2+1)
                for i in range(s_len-1):
                    s2 = (s2[0]+1, s2[1]+1)
                    x, y = s2
                    step1_diagonal_grid[x][y] = rect2_color
                answer_grid = step1_diagonal_grid.copy()
                answer_grid[0, :] = border_color
                answer_grid[h-1, :] = border_color
                answer_grid[:, 0] = border_color
                answer_grid[:, w-1] = border_color
                test_answer_grid = rand_grid.copy()
                test_answer_grid[0, :] = border_color
                test_answer_grid[h-1, :] = border_color
                test_answer_grid[:, 0] = border_color
                test_answer_grid[:, w-1] = border_color
                if (j == num_examples):
                    ...
                    pr_out.append(test_answer_grid)
                    j += 1
                else:
