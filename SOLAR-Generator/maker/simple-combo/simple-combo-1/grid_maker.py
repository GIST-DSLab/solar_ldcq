...
class GridMaker(BaseGridMaker):
    ...
    def parse(self, **kwargs) -> List[Tuple[List[NDArray], List[NDArray], List[NDArray], List[NDArray], Dict]]:
        ...
        while num < num_samples:
            ...
            rect1_color = colors[0]
            rect2_color = colors[1]
            ...
            while (j < num_examples+1):
                ...
                step1_grid[0, :] = border_color
                step1_grid[h-1, :] = border_color
                step1_grid[:, 0] = border_color
                step1_grid[:, w-1] = border_color
                ...
                for i in range(f_len):
                    s1 = (s1[0]-1, s1[1]-1)
                    x, y = s1
                    answer_grid[x][y] = rect1_color
                s_len = min(h-x2-1, w-y2-1)
                s2 = (x2+1, y2+1)
                for i in range(s_len-1):
                    s2 = (s2[0]+1, s2[1]+1)
                    x, y = s2
                    answer_grid[x][y] = rect2_color
                if (j == num_examples):
                    ...
                    j += 1
            desc = {'id': f'simple-combo-1-expert_{num}',
